...
load('model\Tentacle1_model.mat');  % Load model paramters into the workspace
...
Bco = zeros(3*Tentacle1.nsig, length(Ts));             % Body coordinate list

% Simulate Dynamics of Soft Body
for i = 1:(length(Ts)-2)

    % Calculate Dynamic state vector
    [t, qqd] = Tentacle1.dynamics(qqd(end, :), actuation, dt = dt, ...
