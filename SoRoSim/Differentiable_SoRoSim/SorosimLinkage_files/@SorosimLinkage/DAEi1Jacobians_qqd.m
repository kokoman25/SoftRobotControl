...
%dtau_dq and dtau_dqd are initialized later
...
if Linkage.nCLj>0
    ...
    for iCLj=1:Linkage.nCLj %loop over all closed-chain joints
        ...
        % e = A*qdd+Ad*qd+2/T*A*qd+2/T^2*e = Phi_p'(Adj_gBA*etad_A - etad_B - adj_eta_B*Adj_gBA*eta_A + 2/T*(Adj_gBA*eta_A - eta_B) + 1/T^2*OmegaBA)
        de_dq(il_start:il_start+nl-1,:) = Phi_p'*(-dinamico_adj(etad_A)*diffJAB + diffdetad_dqAB + dinamico_adj(eta_A)*deta_dqB+dinamico_adj(eta_B)*dinamico_adj(eta_A)*diffJAB-dinamico_adj(eta_B)*deta_dqA...
                                         ...
                                         +1/(T_BS^2)*(T_RodBA\diffJAB)); %verified
                    ...
                    for ii=1:nip
                        if Ws(ii)>0
                            ...
                            for ia =Linkage.i_sact{i}{j}
                                ...
                                [~,PHI_AU_ia] = SoftActuator(u(n_jact+ia),dc,dcp,xihat_123);
